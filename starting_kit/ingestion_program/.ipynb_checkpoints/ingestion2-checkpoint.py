...
#   image files   -- images in a uniform resolution (128x128)
#   labels.csv    -- file names "FileName" (image names), labels "Labels" (label of each image), subset (train, valid, test) shows if images is in train, validation or test
#   label.name    -- the label names 
#
...
if __name__=="__main__" and debug_mode<4:	
    ...
    data_valid = data[data['subset'] == 'valid']
    data_test = data[data['subset'] == 'test']
    
    
    if not data_train.empty:
        X_train = data_train.drop(['target','subset'], axis=1).to_numpy()
        Y_train = data_train['target']
    else:
        print("#--WARNING--# Train Data NOT Found!")
    
    if not data_valid.empty:
        X_valid = data_valid.drop(['target','subset'], axis=1).to_numpy()
        Y_valid = data_valid['target']
    else:
        print("#--WARNING--# Validation Data NOT Found!")

    if not data_test.empty:
        X_test = data_test.drop(['target','subset'], axis=1).to_numpy()
        Y_test = data_valid['target']
    else:
...
#         continue
    ...
    if os.path.isfile(modelname + '_model.h5'):
        M = M.load(modelname)
        you_must_train=0
        vprint( verbose,  "[+] Model reloaded!")
    # ========= Train if needed only
    if you_must_train:
        vprint( verbose, "#--ERROR--# \n======== Trained model not found! Trained Model should be provided")
    else:
        ...
        filename_valid = data_name + '_valid.predict'                
